...
function getFizzBuzz(num) {
  let res = '';
  if (num % 3 === 0 && num % 5 === 0) { res = 'FizzBuzz'; } else if (num % 3 === 0) { res = 'Fizz'; } else if (num % 5 === 0) { res = 'Buzz'; } else { res = num; }
  return res;
}
...
function getSumBetweenNumbers(n1, n2) {
  return Array(n2 - n1 + 1).fill().map((_, i) => n1 + i).reduce((a, b) => a + b, 0);
}
...
function findFirstSingleChar(str) {
  ...
  const res = [];
  strArr.map((i) => {
    if (strArr.indexOf(i) === strArr.lastIndexOf(i)) {
      res.push(i);
    }
    return true;
  });
